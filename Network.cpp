...
#define GET(name, s, b, bl, t)                                  \
  if ((nbRead = Read(s, (char *)(b), bl, t)) < 0)               \
  {                                                             \
    ::printf("\nReadError(" name "): %s\n", lastError.c_str()); \
    isConnected = false;                                        \
    close_socket(s);                                            \
...
#define PUT(name, s, b, bl, t)                                   \
  if ((nbWrite = Write(s, (char *)(b), bl, t)) < 0)              \
  {                                                              \
    ::printf("\nWriteError(" name "): %s\n", lastError.c_str()); \
    isConnected = false;                                         \
    close_socket(s);                                             \
...
#define GETFREE(name, s, b, bl, t, x)                           \
  if ((nbRead = Read(s, (char *)(b), bl, t)) < 0)               \
  {                                                             \
    ::printf("\nReadError(" name "): %s\n", lastError.c_str()); \
    isConnected = false;                                        \
...
#define PUTFREE(name, s, b, bl, t, x)                            \
  if ((nbWrite = Write(s, (char *)(b), bl, t)) < 0)              \
  {                                                              \
    ::printf("\nWriteError(" name "): %s\n", lastError.c_str()); \
    isConnected = false;                                         \
...
int Kangaroo::Read(SOCKET sock, char *buf, int bufsize, int timeout)
{ // Timeout in millisec
